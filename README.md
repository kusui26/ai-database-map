...
- **スキル**（Claude Code / Cowork / Codex）：分析の型（`station-analysis`）＋用途別レシピ——住宅 `/ai-database-map:recommend`・輸送計画 `:demand`・出店 `:market`——＋ `analyze-csv`・`hazard-reading`。**図を出せるホスト**（MulmoTerminal / MulmoClaude の Canvas）では、要件をフォームで聞き、チャートと地図も出します（無い環境では従来どおりの表と文章）
