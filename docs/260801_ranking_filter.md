...
### 10.4 AI eval（ゴールデン 24 問・`EVAL=1`）— **24/24 合格**

追加問 `rank-operator-shinkansen`（「東海旅客鉄道の新幹線駅で乗降客数が多い順に教えて」）で
**LLM が `operators:["東海旅客鉄道"]` と `routeTypes:[1]` の両方を自力で渡して合格**。
既存 23 問も全問合格（1 回目の通しで 24/24・再試行なし）。

### 10.5 決定 6（⚠除外の既定 ON）の実測
