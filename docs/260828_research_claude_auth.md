...
> **✅ PR-7b 完了（2026-09-03・スキルの汎用化＝CLAUDE.md §1 の想定ユーザー全対応）。**
> #120 マージ後の本番 MCP を golden（housing）1 走で検証（**PASS**・structuredContent 修正の
> 本番反映を実走で確認）。そのうえで方法論を 3 ユースケースへ拡張：
> **`transport-planning`**（輸送計画・ダイヤ検討の需要側材料。ダイヤ・断面輸送量・混雑率・
> 時間帯別は「持っていない」と明言し運行計画を断定しない／乗降＝駅利用であって通過需要では
> ないの推奨文テンプレ／pax 2011–2024 系列＋回復×将来の 4 象限）と
> **`market-analysis`**（出店の商圏分析。業種別プリセット＝按分売上・同業従業者・所得・500m
> 商圏／proxy の明示＝従業者は昼間・競合は同業集積・地価は賃料ではない／2021 調査＝2020 年＝
> コロナ影響年の注記必須）＋コマンド `/demand`・`/market`。プラグイン **0.6.0**。
> **evals**：公式フォーマットの golden ケース 2 本（東横線・カフェ 3 駅）を追加し、
> ローカルランナーを `--scenario housing|transport|market` に汎用化。
> **受け入れ実測**：transport＝調整 2 周（1/3→2/3→**3/3**）・market＝調整 1 周（0/3→**3/3**）。
> 実走がまた**プロダクト改善を 1 件**産んだ——路線名の綴り違いで `list_stations` が
> **無言の 0 件**になり、モデルが名称当てずっぽうで 4 連呼していた → **0 件時の構造化 note**
> （「路線名は会社名を含まない形・0 件が続けば operators で全駅」）を rank と同じ流儀で追加
> （tool-specs・再走で list 呼び出しが 1 回に収斂）。採点側の学び 2 件も記録：
> ①少数駅（〜5 駅）の並記比較は build_dataset の preview から直接組んでよい（方法論に明文化・
> Bash 必須を外す）②「少数回」の判定はデータ系ツールに限定（get_metrics_catalog は軽量な
> メタ照会＝self-describing API の推奨経路であり数えない）。

> **✅ PR-7c 完了（2026-09-03・スキルのリファクタ＝骨格抽出＋レシピ痩身）。**
> 「1 本のスキルが理想では」という問いを受けた再設計（会話で合意した折衷）：
> 意思決定支援の共通骨格を **station-analysis の「分析の型（8 段）」**として 1 か所に抽出
> （①要件 1 回→既定宣言で完走 ②まとめ呼び・0 件 note ③build 1 回・カタログは category 単位
> ④欠損・フラグ・uncovered ⑤正規化明記・災害非線形 ⑥±20% ⑦表＋要因/弱点・少数駅は preview
> 直組み可 ⑧限界＋出典）。用途 3 スキルは**7 行ダイジェスト＋用途の差し込みだけの薄いレシピ**
> （質問リスト・指標プリセット・固有の限界語彙・禁じ手）へ痩身——3 重複を解消しつつ、
> 発火精度（description ルーター）とコンテキスト経済（読む量は用途ぶんだけ）は維持。
> ダイジェストは骨格が同時に読み込まれない実行でも崩れないための保険（静的テストで固定）。
> **挙動同等の実証**：リファクタ後に golden 全通し **housing 5/5・transport 3/3・market 3/3
