...
> **✅ 追補（2026-09-24）：`/ai` が携帯で読めていなかった。** PR-15e の実レンダで見つけた
> 「ツール一覧が 390px で 42px はみ出す」を直した。**症状は「少しはみ出す」ではなかった**——
> いちばん長い札（`get_station_detail / rank_stations / compare_growth`・380px）に `shrink-0` が
> 付いていて縮みも折り返しもせず、**説明が画面の外**に出ていた。320px では説明が 44px 幅の柱になり、
> 1 行に 2〜3 文字しか入っていない。**430px の端末でも溢れていた**（1 行に 448px 要るため）。
>
> 直し方は「狭い画面では**縦に積む**」——`sm:` 未満は `flex-col`、札は `max-w-full` で
> `/` の前後から折り返し、`sm:` 以上は `shrink-0` を戻す。
>
> **広い画面が変わっていないことを、印象ではなく座標で示した**——**本番（旧コード）と手元（新コード）を
> 同時に開き、8 行それぞれの札と説明の矩形を突き合わせる**。1280 / 768 / 640px で
> **完全に一致**（節の高さも 242px で同じ）。狭い側は意図どおり変わり、節の高さは
> 682 → 490px（390px）と**短くなった**——読めるようになったぶん縦が減っている。
>
> 再発防止は `tests/ui.ai-page.smoke.py`（**新設**・7 幅 × 5 項目）。
> **旧コードの本番に当てて 11 件落ちる**ことを確認した。`/ai` は
> 「カタログに載らない以上ここが唯一の入口」（`docs/260915_week5…` §2）なので、
> 携帯で読めないことは到達性の問題そのものである。

> **✅ 追補の続き（2026-09-24）：日本語の途中に空白が入っていた。** 同じ実レンダが拾った
> もう 1 つ。**JSX のテキストは改行が空白 1 個になる**ので、日本語を折り返した場所が
> そのまま文中の空白になる（「地図の タイル」「合わせて その都度」）。英語なら語の区切りとして
> 正しいので、この仕組み自体は直せない——**折る位置を ASCII や数字の隣にする**しかない。
>
> 構文木（JSXText）から数えると、アプリ全体（TSX 64 ファイル）で **2 件**だった。
> 直し方は 1 件ずつ違う：`/ai` は**空白を消す**（誤り）、About の
> 「国土数値情報 利用約款」は**空白を残す**——カタログの license が 355 件すべてこの表記で、
> 原典の書き方だからである。
>
> **ここで 1 つ学んだ**：意図した空白を**直接の空白で書くと prettier がそこで折り返す**。
> 折り返された瞬間、ソースの見た目は「ただの改行」になり、意図は消える。だから
> **意図した空白は `{' '}` で書く**（式なので折り返しでも残り、読む人にも伝わる）。
>
> 見張りは `tests/jsx-text-spacing.test.ts`（**新設**・CI で走る）。**描画結果ではなく
> ソースの構文木**を見る——画面の文字列にはデータ由来の空白（「国土交通省 国土数値情報」など
> カタログの表記）が混ざり、それは直す対象ではないからである。検査するのは
> **改行に由来する空白だけ**で、直接書いた空白と `{' '}` は通す。
...
> **W6 は見送り、代わりに引き渡しだけを入れた（2026-09-16）。**
>
> W6 を保留にしても、**放っておけない穴が 1 つ**あった。チャットのシステムプロンプトには
> おすすめ・重み・合成の記述が 1 行も無く、チャットが持つ `rankStations` は**単一指標**である。
> 「横浜市でファミリー向けのおすすめは？」に対して、単一指標で並べたものを「おすすめ」として
> 返しうる——正規化も重みも災害の扱いも敏感度も限界も無いまま。**機能が無いことより、
> 同じアプリの別の口が規範（§13.4）を破ることのほうが害が大きい。**
>
> そこで、プロンプトに 6 行の規約を足した。①複数指標をまとめた推薦は**自分で順位を作らない**
> ②**画面下の「おすすめ」ボタン**へ引き渡す（質問にエリアがあれば添える）
> ③**単一指標の問いは今までどおり答えてよい**（規則を広げすぎない）。
> 案内できる軸は**プリセットから引く**——手で並べると、指標を足したときにプロンプトだけ古くなる。
>
> **実測が 1 つ直させた。** 軸を書かずに流したら、チャットが「人口・**子育て環境**・利便性」と
> 案内した——**収録していない指標**である。軸を名指しし、「子育て環境・治安・学区・物件価格は
> 案内文に混ぜない」と明記したら消えた。実際の応答：
>
> > ファミリー向けのおすすめの駅は、単一の指標ではなく複数の要素を総合的に見る必要があるため、
> > 画面下の「おすすめ」ボタンをご活用ください。横浜市エリアを指定して、将来人口・実績人口・
> > 地価トレンド・地価水準・乗降回復・乗降水準のバランスから、ご希望に合う駅を一覧・地図・比較で
> > ご覧いただけます。
>
> **検証**：ゴールデンに `refuse-composite-recommend` を追加（**拒否＝1 問も落とせない分野**）。
> 問が 37 → 38 になったので、**許す揺らぎを 2 問のまま**にするため合格ラインを 35 → 36 へ引き直した
> （据え置くと静かに緩む——前回それで 67% まで緩んでいた）。実走 2/2 パスで、
> 単一指標のランキング（`rank-covid-kanagawa`）が壊れていないことも同時に確認した。
> プロンプトから規約が消えていないことは `tests/ai-support.test.ts` が CI で見る。
>
> **W6 を再開する条件**：チャットでおすすめを聞かれている事実が出たとき。そのときも
> 専用パネルは新設せず、既存の `markdown` パネル＋`highlightStations`（どちらもプロトコルに既にある）
> で出せる見込み——`protocol.ts` と母艦へ配る viewer を触らずに済む。
