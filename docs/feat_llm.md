...
- ⤢ とキャンバスの自動表示の照合には、**パネルを生みうる呼び出しだけ**を使う（`panelGroups.toolCallsOf`）。以前は失敗した
  呼び出し（ツールが失敗を結果として返した・実行が投げた）も残っていたので、「千葉市」で失敗して「千葉県」で呼び直し、正しい図が
  出ても、⤢ もキャンバスも「千葉市」で開いて「データがありません」になっていた（2026-09-26 修正・`tests/ui.chat-promotion.smoke.py`）。
- 残る限界：**同じ指標で成功した呼び出しが 2 つある**と（例：事業者名を誤って「該当 0 件」の図が出たあと、絞り込みを外して呼び直した）、
  どちらの図も**最初の**呼び出しの条件で開く。照合はラベルが合う最初の呼び出しを採るため。キャンバスが自動で開く最後の図も同じ。
