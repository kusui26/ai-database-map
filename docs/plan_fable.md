...
| **P7a** | ✅ 完了 | 2026-07-09 | 品質・仕上げ。**About/データ出典ダイアログ**：`dataSources()`（catalog の source/license を集約する純関数＋test）→ 出典表を自動生成（S12/国勢調査/L01/P11+P36/経済センサス を網羅・指標数・カテゴリ・ライセンス文言・**⚠非商用**〔バス2010分母〕を明示）＋地理院クレジット。ヘッダに ⓘ トリガ（初回オープンで遅延ロード）。**メタデータ/OGP/favicon**：title template・openGraph・twitter・metadataBase・robots・viewport(themeColor)／`icon.svg`／`opengraph-image.tsx`（next/og・ラテン文字で tofu 回避・自己完結）。**エラー/ロード/404/オフライン**：`error.tsx`・`global-error.tsx`・`not-found.tsx`・`loading.tsx`＋`OfflineBanner`（online/offline 監視）。**a11y**：グローバル `:focus-visible` リング・地図に `role=application`＋aria-label・`main` ランドマーク。**Lighthouse 改善**：`useSearchParams`（nuqs）による **CSR バイアウト**を解消——Suspense の fallback を実体シェル（`MapSkeleton`＝main＋スケルトンをサーバ描画）に。ダイアログ/駅詳細/About を **コード分割**（初回オープン/選択まで遅延・Chart.js を初期バンドルから除外）。タイル host へ **preconnect**。→ FCP 1.8→0.8s・SI 1.8→1.1s・TBT 290→~200ms・CLS 0。**Lighthouse mobile：Perf 78・A11y 98**／**desktop：Perf 98・A11y 98（LCP 1.1s）**。A11y は目標(≥90)超過。mobile Perf は目標(≥80)を2点下回るが、唯一の弱点は 4x-CPU/低速4G 模擬下の **MapLibre キャンバス LCP（~5.2s）**で、9千駅 WebGL 地図の本質的制約（実測 LCP 160ms・desktop LCP 1.1s）。ヘッドレス検証：検索→詳細→ランキング→散布→About→404 全フローでエラー画面なし・出典網羅・console error 0・test(99)・build 緑 |
| **P7b** | ✅ 完了 | 2026-07-09 | 公開準備。**Vercel Cron**（`vercel.json`＝`0 3 * * *`→`/api/health`）で Supabase 無料枠の自動停止を防止（`/api/health` は DB 1 クエリ＝`{ok:true}` を実測確認）。**README.md** 新規（概要・「API こそがプロダクト」・mermaid アーキ図・主な機能・技術スタック・セットアップ・データ出典/ライセンス表・構成・デプロイ・ステータス）。**architecture.md** §7.5 を実体（P0–P7b 完了・src/domain/db/shared/components・supabase migrations・9,273駅/503万行・Lighthouse 78/98・Vercel+Cron）に更新、§8 に Cron・`NEXT_PUBLIC_SITE_URL` を反映。`.env.example` に `NEXT_PUBLIC_SITE_URL` 追記。typecheck/lint/test(99)/build 緑。**本番 env 設定・デプロイ・cron ログ確認・カスタムドメインは Vercel ダッシュボード（ユーザー作業）**。Step1 DoD：地図/検索/詳細/半径/ランキング/散布/出典/モバイル・共通API・Supabase(RLS/全数照合)・docs は達成、Lighthouse mobile Perf は 78（目標80／地図キャンバス LCP が律速・desktop 98） |
