...
0 は該当なし。該当判定は「**250m セルの代表点（中心）が区域に入るか**」（v1・地図に描く公式タイルと一致させるため）。

> ⚠ **v2 への変更が決まっている**（`docs/260824_flood.md` §5.9・§8.2b・未実装）。
> セル 1 個に 1 値だと、点で聞かれたときに **7.7% 誤答**する（混在セルが 33% あるため）。
> **1 バイト＝上位ニブル「セル内の最大ランク」＋下位ニブル「被覆率 0–15」**に改め、
> 両端（0＝一切かからない／15＝全域）だけを厳密な主張に使う。ラスタ化は
> `1×all_touched`（最大）＋ `8×サブセル`（被覆率）の 2 パスになる（コストは現状と同じ・実測済み）。
