...
2. **データセット生成**：`build_dataset` に
   ...
   `get_hazard_summary`（≤500 駅・事前計算）を使う。
3. **取得**：pandas は URL を直接読める。**meta も必ず読む**（単位・年次・半径・出典・注意）。
   ...
   **置き場所**：母艦（MulmoTerminal / MulmoClaude）なら `artifacts/data/`、
   Claude Code 単体なら `./data/`。最後に「何をどこに保存したか」を 1 行で伝える。
...
9. **図を出せる環境なら図も出す**：`presentChart` などがあるときは、CSV の分析結果に加えて
   サーバが作った option をそのまま渡す（[station-analysis の Canvas 節](../station-analysis/SKILL.md)）。
   自分で option を書き直さない。
