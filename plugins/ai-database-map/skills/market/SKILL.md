...
3. 比較表＋強み/弱み → 限界（按分推計・2020 年はコロナ影響・従業者＝昼間 proxy・
   地価は賃料ではない）＋出典
4. **図を出せる環境なら**：駅ごとの図は `get_station_detail` に `present: "echarts"` を付けて
   返った option を**そのまま** `presentChart` に。比較の図を自分で作るなら脚注を自分で添える。
   地図は候補駅を `[{"type":"highlightStations","grps":[…]}]` で `render_map` → 保存 → `presentHtml`。
