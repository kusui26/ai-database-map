...
/** カンマ区切り → 空要素を除いた配列（未指定は undefined＝絞らない）。 */
...
/** カンマ区切りの整数（事業者種別）。整数以外は 400。 */
function parseTypes(raw: string | undefined): number[] | undefined {
  const names = parseNames(raw)
  if (names === undefined) return undefined
  const types = names.map(Number)
  if (types.some((type) => !Number.isInteger(type))) {
    throw new BadRequestError(`routeTypes が不正です: ${raw ?? ''}`)
  }
  return types
}
