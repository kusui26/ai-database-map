        ...
        >
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-4 py-4 text-sm text-slate-700">
            <section className="space-y-2">
              <p>
                <span className="font-semibold text-slate-900">AI Database Map</span> は、公的オープンデータ（乗降客数・人口・地価・バス・事業所・従業者）を
                <span className="font-medium">駅からの半径</span>で集約し、地図とAIで誰でも扱えるようにする実験的な Web アプリです。
              </p>
              <p className="text-slate-500">
                数値は各公的統計の二次加工であり、原典の定義・年次・集計単位に依存します。低分母など信頼性に注意すべき値には ⚠ を付しています。
              </p>
            </section>
            ...
            <section className="space-y-2">
              ...
              <p className="text-xs text-slate-500">
                ...
                利用約款）ですが、<span className="font-medium text-amber-700">⚠</span> の付いた出典は商用利用に制限があります。ご利用の際は各原典の利用規約をご確認ください。
              </p>
              <ul className="space-y-2">
                {sources.map((s) => (
                  ...
                  >
                    ...
                    <p className={cn('mt-1.5 text-xs', s.nonCommercial ? 'text-amber-700' : 'text-slate-500')}>
                      {s.license}
                    </p>
                  </li>
                ))}
