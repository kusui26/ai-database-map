 ...
 *
 * 呼ぶ側（`/api/recommend`・W3）はここを 1 回叩くだけでよい。順番と往復の回数を
 * 呼び出し側に持たせない——持たせると、画面とチャットで別の順番になりうる。
 *
 * 返すのは順位だけではない。**埋めた既定（`notes`）・解決できなかった指定（`unresolved`）・
 * 対象集合の件数（`stationCount`）**も一緒に返す。脚注に書くものが揃っていないと、
 * 出てきた順位は読めない。
 */
...
import type { FlaggedPolicy, HazardPolicy, NormalizeMethod, RecommendResult } from './types'
...
export async function runRecommendation(input: RecommendRunInput): Promise<RecommendRun> {
  ...
  return { result, notes, unresolved, stationCount: gathered.stationCount }
}
